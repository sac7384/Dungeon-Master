    ...
    while(true){
      ...
      //Inventory
          if (action==5){
            ...
            System.out.println("Enter the number of the item you would like to use or equip. Enter any other number to exit.");
            for (int i=0; i < inventory.size(); i++){
              System.out.println(i+1+"-"+inventory.get(i));
            }
            int x=scan.nextInt();
            if (x-1 >= 0 && x-1 < inventory.size()) {
              if (inventory.get(x-1).getPurpose()==0){
                if (inventory.get(x-1).getClass() == p.getWeapon().getClass()) {
                  System.out.println("You equipped the "+inventory.get(x-1).getName()+" and put the "+p.getWeapon().getName()+" in your inventory.");
                  p.addItem(p.unequipWeapon());
                  p.equip((Weapon)inventory.get(x-1));
                  p.removeItem(x-1);
                }
                if (inventory.get(x-1).getClass() == p.getArt().getClass()) {
                  System.out.println("You equipped the "+inventory.get(x-1).getName()+" and put the "+p.getArt().getName()+" in your inventory.");
                  p.addItem(p.unequipArt());
                  p.equip((Artifact)inventory.get(x-1));
                  p.removeItem(x-1);
                }
                if (inventory.get(x-1).getClass() == p.getScroll().getClass()) {
                  System.out.println("You equipped the "+inventory.get(x-1).getName()+" and put the "+p.getScroll().getName()+" in your inventory.");
                  p.addItem(p.unequipScroll());
                  p.equip((Scroll)inventory.get(x-1));
                  p.removeItem(x-1);
                }
                if (inventory.get(x-1).getClass() == p.getArmor().getClass()) {
                  System.out.println("You equipped the "+inventory.get(x-1).getName()+" and put the "+p.getArmor().getName()+" in your inventory.");
                  p.addItem(p.unequipArmor());
                  p.equip((Armor)inventory.get(x-1));
                  p.removeItem(x-1);
                }
                x=inventory.size()-1;
              }
                ...
                System.out.println("You heal yourself for "+p.heal(inventory.get(x-1).getValue()));
                p.removeItem(x-1);
              }
              
              if (inventory.get(x-1).getPurpose()==2){
                System.out.println("You damage the enemy for "+inventory.get(x-1).getValue());
                e.heal(-1*inventory.get(x-1).getValue());
                p.removeItem(x-1);
              }
